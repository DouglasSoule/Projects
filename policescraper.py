...
most_recent_file = sorted(os.listdir('data/html'))[-1]
# print(most_recent_file)
fhtml = codecs.open(f'data/html/{most_recent_file}', 'r')
...
for incident_key in incidents.keys():
    ...
    incidents[incident_key]['case_no'] = incident_key
...
with open(f'data/incidents_chunks/incidents_{date.today()}.csv', 'w+') as outfile:
    col_names = []
    for incident_key in incidents.keys():
        col_names = list(incidents[incident_key].keys())
        break
    ...
    for incident_key in incidents.keys():
        writer.writerow(incidents[incident_key])

with open('data/all_incidents/master_file.csv') as master_file:
    data_file = csv.DictReader(master_file)
    keys = [row['case_no'] for row in data_file]
    # keys = [row['case_no'] for row in data_file]

with open('data/all_incidents/master_file.csv', 'a+') as master_file:
    writer = csv.DictWriter(master_file, fieldnames=col_names)
    for incident_key in incidents.keys():
        # print(keys)
        if incidents[incident_key]['case_no'] not in keys:
            writer.writerow(incidents[incident_key])
...
# =======
#   # python imports
# >>>>>>> d57a2d7fb442c27f8a7d4063382755648cc67ae3
# import json
# import csv, json, sys
